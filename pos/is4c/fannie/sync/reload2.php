...
function synctable($table) {
	...
	exec('/usr/local/mysql/bin/mysqldump -u '.$serveruser.' -t '.is4c_op.' '.$table.' | /usr/local/mysql/bin/mysql -h '.$lane.' '.is4c_op." 2>&1", $result, $return_code);
	foreach ($result as $v) {$output .= "$v\n";}
	if ($return_code == 0) {

		return 1;
	} else {
		syslog(LOG_WARNING, "uploadtable($table) failed; rc: '$return_code', output: '$output'");
		return 0;
	}
}
